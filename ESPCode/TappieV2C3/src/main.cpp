...
void IRAM_ATTR readEncoderISR()
{
  ...
  // Read both pins at the same time to avoid race conditions
  uint8_t clk = digitalRead(ENCODER_PIN_CLK);
  uint8_t dt = digitalRead(ENCODER_PIN_DT);

  // Combine into a state value 0-3
  uint8_t newState = (clk << 1) | dt;

  // Only process the state if it's different from the last one
  // This avoids spurious triggering
  if (newState != lastState)
  {
    ...
    // Most rotary encoders produce 4 state transitions per detent
    // We'll set our threshold accordingly
    if (counter >= 4)
    {
      encoderDelta++; // One complete CW detent
      counter = 0;
      fullDetentRotation = true;
    }
    else if (counter <= -4)
    {
      encoderDelta--; // One complete CCW detent
      counter = 0;
      fullDetentRotation = true;
    }

    lastState = newState; // Save the state for next comparison
  }
}
...
void setup()
{
  ...
  attachInterrupt(ENCODER_PIN_CLK, readEncoderISR, CHANGE); // Attach interrupt to CLK pin
  attachInterrupt(ENCODER_PIN_DT, readEncoderISR, CHANGE);
...
void loop()
{
  ...
  if (fullDetentRotation)
  {
    int8_t delta;

    // Safely get and reset the encoder delta
    noInterrupts();
    delta = encoderDelta;
    encoderDelta = 0;
    fullDetentRotation = false; // Reset the flag
    interrupts();

    if (delta > 0)
    {
      Serial.println("+");
      String positionStr = "increasing" + getBatteryLevel();
      encPosChara->setValue(positionStr.c_str());
      encPosChara->notify();
    }
    else if (delta < 0) // Explicitly check if delta < 0
    {
      Serial.println("-");
      String positionStr = "decreasing" + getBatteryLevel();
      encPosChara->setValue(positionStr.c_str());
      encPosChara->notify();
    }

    // Update last activity time since we had encoder movement
    lastActivityTime = millis();
  }
